...
public:
   ...
   virtual  ~VOT() { }
    ...
    virtual void init(const cv::Mat& image, const cv::Rect &roi, const cv::Mat &mask =cv::Mat(), const bool& USE_MASK=false) = 0;
    virtual cv::Rect update(cv::Mat& image)=0;
	virtual cv::Rect update(cv::Mat& image, float& confProb) = 0;
	virtual cv::Rect update(const cv::Mat& image, float& confProb,
		...
		const cv::Mat &mask = cv::Mat(), const bool& USE_MASK = false) = 0;
